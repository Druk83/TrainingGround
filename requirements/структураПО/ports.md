...
| Python Generator / Explanation API | 8000 | REST/gRPC | Только внутренняя сеть; таймаут 2 сек. Запланирован в A3. |
| MongoDB Replica Set (Primary) | 27017 | Документное хранилище | Обязательный порт. |
| MongoDB Replica Set (Secondary1) | 27018 | Репликация данных | Для dev окружения; в production за internal network. |
| MongoDB Replica Set (Secondary2) | 27019 | Репликация данных | Для dev окружения; в production за internal network. |
| Redis Cluster | 6379 | Кэш, очереди | Обязательный. |
| Qdrant (REST API) | 6333 | Векторный индекс | Обязательный. |
| Qdrant (gRPC) | 6334 | Векторный индекс (gRPC) | Дополнительный endpoint для gRPC клиентов. |
...
- Все внутренние порты (8081, 8000, 27017-27019, 6379, 6333-6334) доступны только в приватной сети/Service Mesh (mTLS).
- MongoDB Replica Set порты 27018-27019 открыты в dev окружении для локального доступа; в production доступны только внутри кластера.
