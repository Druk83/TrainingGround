...
from explanation_service.models.dto import ExplanationRequest, ExplanationResponse  # noqa: E402
from explanation_service.services.cache import ExplanationCache  # noqa: E402
...
async def test_cache_roundtrip() -> None:
    ...
    request = ExplanationRequest(task_id="task-1", topic_id="topic-1", user_errors=["ошибка"])
    response = ExplanationResponse(explanation="hello", rule_refs=["r1"], source="yandexgpt", took_ms=10)
...
async def test_cache_miss_on_different_payload() -> None:
    ...
    response = ExplanationResponse(explanation="foo", rule_refs=[], source="fallback", took_ms=5)
