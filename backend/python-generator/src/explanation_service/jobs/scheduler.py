...
class MaintenanceScheduler:
    ...
    def __init__(
        self, settings: Settings, qdrant: QdrantClient, redis_client: Redis
    ) -> None:
    ...
    def start(self) -> None:
        ...
        LOGGER.info(
            "Maintenance scheduler started with cron %s", self._settings.snapshot_cron
        )
    ...
    async def _snapshot_job(self) -> None:
        LOGGER.info(
            "Creating Qdrant snapshot for %s", self._settings.qdrant_rules_collection
        )
