...
## Чек-лист готовности
- [ ] Все коллекции MongoDB имеют описанные поля, индексы, TTL и политику шифрования PII.
- [ ] Lua-скрипты и конфигурация Redis задокументированы, а ключи автоматически создаются/очищаются.
- [ ] Qdrant коллекции и snapshot flow можно поднять заново по инструкции без ручных правок.
- [ ] Потоки MongoDB → Redis/Redis Streams протестированы на идемпотентность и устойчивость к сбоям.
- [ ] Бэкапы регулярно выполняются и проверяется процедура восстановления.
- [ ] Документация и схемы синхронизированы с фактическими конфигурациями.
