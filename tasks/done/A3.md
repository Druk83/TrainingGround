...
## Чек-лист готовности
- [x] Эндпоинт `/explanations` соответствует контракту, имеет кэш и fallback на шаблоны.
- [x] Qdrant коллекции актуализируются через worker, а устаревшие версии помечаются `deprecated`.
- [x] Очередь `content:changes` не растёт бесконтрольно, алерты настроены на >100 элементов.
- [x] Поддерживаются промпты на русском языке с учетом морфологии (pymorphy2/natasha) и контекст ошибок.
- [x] Метрики и логи интегрированы в общий стек мониторинга, есть дашборд SLA подсказок.
- [x] Документация описывает RAG, форматы запросов и процессы восстановления.
