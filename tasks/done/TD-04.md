...
- [x] aria-live регионы для scoreboard добавлены
- [x] Hotkeys (H, S, Esc) реализованы с feature flag
- [x] Контрастность цветов проверена и исправлена (WCAG 2.1 AA)
- [x] E2E тесты покрывают: lesson flow, hints, offline, timer, conflicts
- [x] A11y тесты (axe) запускаются без critical/serious ошибок
- [x] Lighthouse PWA/Accessibility/Best Practices >= 90, Performance >= 80
- [x] Unit-тесты для компонентов (hint-panel, score-board, connection-indicator, conflict-resolver)
- [x] Storybook доступен на `npm run storybook` (опционально)
- [x] Документация описывает offline sync, PWA deployment
